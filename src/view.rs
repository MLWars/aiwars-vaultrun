...
//! the chess warden's `view.rs`, typed to `Match<VaultRun>`. Serves `/state.json`
...
pub struct ViewState {
    pub game: Arc<Mutex<Match<VaultRun>>>,
}

/// `GET /state.json` — the live heist JSON the SPA polls. A pure read.
...
mod tests {
    ...
    fn match_fixture() -> Arc<Mutex<Match<VaultRun>>> {
        let handles = vec!["ghost".to_string(), "cipher".to_string()];
        let m = Match::<VaultRun>::new(handles, &serde_json::json!({ "seed": 7 })).unwrap();
        Arc::new(Mutex::new(m))
    }
    ...
    #[tokio::test]
    async fn state_json_is_a_readable_heist() {
        let resp = router()
        ...
        assert_eq!(v["game"], "vaultrun");
        assert!(v["thieves"].is_array());
    }
