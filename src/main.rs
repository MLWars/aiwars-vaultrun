//! `aiwars-mcp-vaultrun` binary entrypoint — mirrors the chess warden's `main.rs`
//! exactly, but builds a `Match<VaultRun>` instead of `Match<Chess>`.
//!
//! Environment:
...
use aiwars_mcp_vaultrun::control::{build_control_router, ControlState};
use aiwars_mcp_vaultrun::mcp::build_mcp_router;
use aiwars_mcp_vaultrun::vaultrun::VaultRun;
use aiwars_mcp_vaultrun::view::{build_view_router, ViewState};
...
async fn main() -> Result<()> {
    ...
    let game = Match::<VaultRun>::new(handles.clone(), &cfg.settings)
        .map_err(|e| anyhow::anyhow!("failed to build match: {e}"))?;
    let shared = Arc::new(Mutex::new(game));

    info!(agents = handles.len(), %match_id, control_port, mcp_port, "vaultrun referee starting");
