//! The MCP server for the Vault Run referee. A faithful copy of the chess warden's
//! `mcp.rs`, typed to `Match<VaultRun>`: a SINGLE bearer-gated `/mcp` endpoint
...
//! path (e.g. `sneak:hall`) instead of a UCI move.
...
use crate::vaultrun::VaultRun;
use aiwars_mcp_warden::game::Match;

/// Shared, mutex-guarded match state shared across all agents' requests.
pub type SharedMatch = Arc<Mutex<Match<VaultRun>>>;

/// `make_move` tool arguments.
#[derive(Debug, Deserialize, JsonSchema)]
pub struct MakeMoveArgs {
    /// The path to take, one of your legal moves (e.g. "sneak:hall",
    /// "dash:gallery", "distract:guard", "grab:vault").
    pub mv: String,
...
impl RefereeHandler {
    #[tool(
        description = "Get the current heist state: your room/lane, distract charges, whether the gallery ahead is lit, the rival, status, winner, and your legal paths.",
    ...
    #[tool(
        description = "List the paths you may take now (sneak:hall / dash:gallery / distract:guard / grab:vault) and whether it is your turn, with the current ply.",
        annotations(read_only_hint = true, open_world_hint = false)
    )]
    ...
    #[tool(
        description = "Take a path. Pass the ply you saw as expected_ply. sneak:hall is slow but shadowed; dash:gallery is fast but caught if a cone is lit; distract:guard freezes a sweep; grab:vault wins when adjacent. Returns the new state on success; an error (no change) if illegal, out of turn, or stale.",
    ...
    #[tool(
        description = "Resign the run (forfeit). Your rival is awarded the win.",
        annotations(read_only_hint = false, destructive_hint = false, open_world_hint = false)
    )]
...
impl ServerHandler for RefereeHandler {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions(
            "AIWars Vault Run referee. Call get_state / legal_moves to observe, make_move to take a \
             path (with the ply you saw), resign to forfeit. Slip past the guards' sweeping vision \
             cones to the vault: sneak:hall is slow but safe, dash:gallery is fast but caught if a \
             cone is lit, distract:guard freezes a sweep, grab:vault wins when adjacent. Crack the \
             vault first. Your seat is identified by your bearer token; you cannot act as your rival.",
        )
    }
}
